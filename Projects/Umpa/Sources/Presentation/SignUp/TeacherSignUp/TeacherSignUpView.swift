...
struct TeacherSignUpView: View {
  ...
  init(socialLoginType: SocialLoginType) {
    _signUpModel = StateObject(wrappedValue: TeacherSignUpModel(socialLoginType: socialLoginType))
  }
  ...
  private func validateInput(of signUpStep: TeacherSignUpStep) -> Bool {
    switch signUpStep {
    case .majorSelection:
      signUpModel.validateMajor()
    case .profileInput:
      signUpModel.validateGender() && signUpModel.validateLessonRegion()
